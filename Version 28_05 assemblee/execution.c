...
void execution(S_question questions[],int nb_question,int mode)
{
   ...
   // int note; //servira à compter les points
    ...
    if(mode==0)
    {
        for(j=0;j<nb_question;j++)
            {NETTOYER();
                ...
                printf("Solution : %i", questions[j].solution);
                printf("\nombre de points : %i", questions[j].points);
    ...
    else
    {
        ...
        fprintf(fichier_resultat,"Résultats de l'eleve NOM :\n");
        ...
        for(j=0;j<nb_question;j++)
            {
                        ...
                        fprintf(fichier_resultat,"%i) Reponse attendue : %i -- Réponse donnee : %i\n",j,questions[j].solution,reponse);
        ...
        printf("Vous avez mis %f secondes pour resoudre cet exercice !", difftime(temps_ecoule, temps_initial));
    }
}
